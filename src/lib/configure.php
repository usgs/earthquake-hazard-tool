<?php
  ...
  if ($configure_action === '2' || $configure_action === '3') {
    ...
    $curveServices = array();
    foreach (explode(',', $CONFIG['CURVE_SERVICES']) as $serviceName) {
      $curveServices[] = configureWebService($serviceName);
    }
    $CONFIG['CURVE_SERVICES'] = implode(',', $curveServices);

    $deaggServices = array();
    foreach (explode(',', $CONFIG['DEAGG_SERVICES']) as $serviceName) {
      $deaggServices[] = configureWebService($serviceName);
    }
    $CONFIG['CURVE_SERVICES'] = implode(',', $deaggServices);
